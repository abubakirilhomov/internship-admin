...
//       // Handle nested data (e.g., { data: [...] }) as seen in Dashboard
//       const rulesData = Array.isArray(response) ? response : response.data || [];
//       console.log('Rules data:', rulesData); // Debug log
//       setRules(rulesData);
//     } catch (error) {
...
//     const latestFeedback = intern.feedbacks?.[0] || {};
//     const internViolations = intern.violations?.map(v => v.ruleId.toString()) || [];
//     setFormData({
//       name: intern.name || '',
...
//       stars: latestFeedback.stars || 0,
//       lessonId: '',
//       lessonsVisitedCount: '',
//       grade: intern.grade || 'junior'
//     });
//     setSelectedViolations(internViolations);
//     setShowModal(true);
//   };
...
//   const sortedRules = Array.isArray(rules) ? [...rules].sort((a, b) => {
...
//                         {intern.violations?.map((v, index) => {
//                           const rule = rules.find(r => r._id === v.ruleId.toString());
//                           return rule ? (
//                             <span
//                               key={index}
//                               className={`badge ${getCategoryColorClass(rule.category)} text-white`}
//                             >
//                               {rule.title}
//                             </span>
//                           ) : null;
...
//                   <p>Уроки: {(intern.lessonsVisited.length || 0)}</p>
//                   <p>Нарушения: {intern.violations?.map(v => {
//                     const rule = rules.find(r => r._id === v.ruleId.toString());
//                     return rule ? rule.title : 'N/A';
//                   }).join(', ') || 'Нет'}</p>
...
//               </button>
//             </div>
//             {showViolations && rules.length > 0 && (
//               <div className="form-control mb-4">
//                 <label className="label">
//                   <span className="label-text">Нарушения (опционально)</span>
//                 </label>
...
import InternsContainer from "../components/Layout/Interns"
